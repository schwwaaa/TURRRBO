...
## CRITICAL — index.html must be in the project root

`index.html` must exist at the root of the project alongside `package.json`. Without it Vite returns 404 and the Tauri window shows a white screen.
...
If you see a white screen with no console errors, check this first:

```bash
ls "/Users/tgm/Documents/SPLASH/TURRRBO/index.html"
```

If missing, the file contents are:
